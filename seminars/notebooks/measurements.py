...
from pv080_crypto import rsa_encrypt, rsa_decrypt, aes_encrypt, aes_decrypt


def max_bytes_processed_encryption(num_bits: int) -> int:
    return num_bits // 16 - 33


def max_bytes_processed_decryption(num_bits: int) -> int:
    return num_bits // 8


def RSA_split_end_encrypt(key: rsa.RSAPublicKey, plaintext: bytes) -> bytes:
...
def RSA_split_end_decrypt(key: rsa.RSAPrivateKey, ciphertext: bytes) -> bytes:
...
def measure_aes_speed(key: bytes, message: bytes) -> None:
...
def measure_rsa_speed(key: rsa.RSAPrivateKey, message: bytes) -> None:
